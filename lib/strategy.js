...
function JwtStrategy(options, verify) {
	...
	if(!isFunction(this._secretOrKey)) {
		this._secretOrKey = wrapStaticSecretInCallback(this._secretOrKey);
	}
...
JwtStrategy.prototype.authenticate = function (req, options) {
	...
	this._secretCallback(req, decodedToken.header, decodedToken.payload, function (secret_err, key) {
		if(secret_err) {
			return self.fail(secret_err);
		} else {
			// Verify the JWT
			JwtStrategy.JwtVerifier(token, key, this._verifOpts, function (jwt_err, payload) {
				if(jwt_err) {
